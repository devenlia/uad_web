<script lang="ts">
    ...
    import { closeModal, openModal } from "$lib/components/actions/add";
    import { goto, invalidateAll } from "$app/navigation";
    import { addToast } from "$lib/stores/toastStore";

    export let data: LayoutData;

    let parent = {id: "", name: "", path: data.path}
...
<div class="navbar bg-base-200 rounded-box w-full">
    <div class="flex-1">
    ...
    <div class="flex-none">
        <button on:click={() => openModal("select", parent)} class="btn btn-square btn-ghost text-lg"><IconParkOutlinePlus/></button>
        <div class="dropdown dropdown-end">
            <label tabindex="0" class="btn btn-square btn-ghost text-xl"><IconParkOutlineMore/></label>
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                <li><button on:click={deletePage} class={parent.path === "home" ? "btn-disabled" : ""}><IconParkOutlineDelete/><span>Delete</span></button></li>
            </ul>
        </div>
    </div>
</div>
