<script lang="ts">
  ...
  import { Step, Visible, Action, Tutorial } from "./store";
  ...
  let step : number;
  Step.subscribe((value) => {step = value;})

  let actions = [
    {type: "Page", action: () => {Action.set("page")}},
    {type: "Container", action: () => {Action.set("container")}},
    {type: "Category", action: () => {Action.set("category")}},
    {type: "Link", action: () => {Action.set("link")}}
  ]
  ...
  <div class="modal-box w-screen">
    <div class="flex flex-row justify-between align-middle">
      <h1 class="text-lg">
        {
          ...
          action === "link" ? "Add a link to access your application" :
      ...
      <div class="flex flex-row justify-between align-middle gap-1.5">
        ...
        <button on:click={closeModal} class="btn btn-sm btn-ghost btn-square"><IconParkOutlineClose/></button>
      </div>
    </div>
