  ...
  const deleteContainer = async () => {
    ...
    if (res.status != 200) {
      ...
      addToast({
        id: "",
        priority: 2,
        message: "Error while trying to delete a container! See console for more information."
      })
  ...
  <div class="text-2xl bg-base-100 relative w-max px-3 py-1 rounded-box flex flex-row group" style="top: -20px; left: 20px">
    ...
    <div class="dropdown ml-1">
      ...
      <ul tabindex="0" class="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-52">
        <li><button on:click={() => openDeleteModal(deleteContainer)}><IconParkOutlineDelete/><span>Delete</span></button></li>
        <li><button on:click={() => openAddModal("category", null, container.id)}><IconParkOutlinePlus/><span>Add a category</span></button></li>
      </ul>
    </div>
  </div>
        ...
        <div class="flex flex-row h-20 w-full justify-center content-center">
          <div class="chat chat-end w-52 h-max">
            <div class="chat-bubble w-full chat-bubble-accent">
              ...
              <button on:click={() => openAddModal("category", null, container.id)} class="btn btn-outline text-white border-white hover:bg-white btn-sm w-max">Add one!</button>
            </div>
          </div>
